    ...
    <div class="p-5 sm:p-8 md:grid grid-cols-12">
        <div class="col-start-2 col-span-10 lg:col-start-3 lg:col-span-8  xl:col-start-4 xl:col-span-6">
            ...
            <div class="rounded border border-gray-300 overflow-hidde">
                ...
                <form class="px-5 my-4" action="{{ route('todo.update', ['todo' => $todo->id]) }}" method="POST">
                    @csrf
                    @method('put')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea rows="3" name="description" id="description"
                            class="@error('description') border-red-500 @enderror block w-full border border-gray-300 rounded px-3 py-1 focus:outline-none focus:ring-4 focus:ring-blue-500 focus:ring-opacity-20 focus:border-blue-400">{{ $todo->description }}</textarea>
                        @error('description')
                            <span class=" inline-block mt-2 text-red-600 text-sm"> {{ $message }}</span>
                        @enderror
                    </div>
                    <button type="submit"
                        class="block bg-black text-white border rounded-[3px] leading-none px-3 py-2 border-black text-sm focus:ring-4 focus:ring-black focus:ring-opacity-20">ویرایش
                    </button>
                </form>
            </div>
